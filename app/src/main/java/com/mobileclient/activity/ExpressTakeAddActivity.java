	...
	public void onCreate(Bundle savedInstanceState) {
		...
		spinner_companyObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				...
				Toast.makeText(ExpressTakeAddActivity.this, userInfoList.get(arg2).getUser_name(), Toast.LENGTH_LONG).show();
				t=arg2;
			}
			@Override
		...
		ET_receiverName.setText(declare.getUserName());
...
//		}
			...
			public void onClick(View v) {
				try {
					...
					//if(ET_takeStateObj.getText().toString().equals("")) {
						...
						//return;
					//}
					expressTake.setTakeStateObj("未接单");
					/*验证获取发布时间*/ 
	...
	private  void init() {
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					userInfo=userInfoService.GetUserInfo(declare.getUserName());
				} catch (Exception e) {
					e.printStackTrace();
				}
				expressTake.setUserPhoto(userInfo.getUserPhoto());
			}
		}).start();
